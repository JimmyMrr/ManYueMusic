...
//                    .addHeader("Cookie", "appver=1.5.0.75771");
            ...
            Response response = mOkHttpClient.newCall(request).execute();
            Log.d(TAG, "getResposeJsonObject: " + response.toString());
            if (response.isSuccessful()) {
                String c = response.body().string();
...
//            File sdcache = context.getExternalCacheDir();
            ...
            Request request = builder.build();
            Log.d(TAG, "PostResposeJsonObject: " + request.toString());
            Response response = mOkHttpClient.newCall(request).execute();
            if (response.isSuccessful()) {
                String c = response.body().string();
                Log.d(TAG, "PostResposeJsonObject: " + c);
                JsonParser parser = new JsonParser();
                JsonElement el = parser.parse(c);
                return el.getAsJsonObject();
...
//                CookiePolicy.ACCEPT_ALL));
    ...
    public static void postUrl(Context context, String j) {
        try {
            ...
            Response response = mOkHttpClient.newCall(request).execute();
            if (response.isSuccessful()) {
                Log.d(TAG, "postUrl: " + response.body().string());
            }
    ...
    public static void postNetease(Context context, String j) {
        try {
            ...
            Response response = mOkHttpClient.newCall(request).execute();
            if (response.isSuccessful()) {
                Log.d(TAG, "postNetease: " + response.body().string());
            }
